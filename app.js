...
function showIntro(){
  const s=h('div','screen ih');const g=h('div','gg');g.textContent='🧤';
  ap(s,g,h('h1','','5 Keeper Checks'),h('p','lede','Geen trainer die je beoordeelt. Jij voelt het zelf. 5 korte checks, elk met een vraag, een beweging, en een volledig 6-weken schema.'));
...
function showProg(){
  ...
  const top=h('div','ht');
  ap(top,h('div','hn','🧤 '+(S.name?S.name+"'s schema":'Jouw schema')),
